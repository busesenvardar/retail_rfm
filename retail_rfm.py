...
# Business Problem
###############################################################
# A wants to segment its customers and determine marketing strategies according to these segments.
...
# TASK 1: Data Understanding and Preparation
           ...
           # 3. Omnichannel means that customers shop from both online and offline platforms. Create new variables for each customer's
                # total purchases and spend.
           # 4. Examine the variable types. Change the type of variables that express date to date.
           # 5. Look at the distribution of the number of customers, average number of products purchased, and average spend in
                # shopping channels.
...
# TASK 5: Time for action!
           ...
           # 2. With the help of RFM analysis, find the customers in the relevant profile for 2 cases and save the customer IDs
# to the csv.
                   # a. FLO includes a new women's shoe brand. The product prices of the brand it includes are above the general
                        # customer preferences. For this reason, customers in the profile who will be interested in the promotion of
                        # the brand and product sales are requested to be contacted privately. Those who shop from their loyal
                        # customers (champions, loyal_customers), over 250 TL on average and from the women category, are the
                        # customers who will be contacted privately. Save the id numbers of these customers
                        # in the csv file as new_brand_target_customer_id.cvs.
                   # b. Up to 40% discount is planned for Men's and Children's products. It is desired to specifically target
                        # customers who are good customers in the past, but who have not shopped for a long time, who are interested
                        # in the categories related to this discount, who should not be lost, those who are asleep and new customers.
...
# 1.
df_ = pd.read_csv("datasets/retail_data_20K.csv")
df = df_.copy()
...
df.describe([0.5, 0.7, 0.85, 0.9, 0.95, 0.99]).T
...
# Segment definition and conversion of RF_SCORE to segments with the help of defined seg_map so that the generated RFM scores
...
cust_ids.to_csv("yeni_marka_hedef_mü?teri_id.csv", index=False)
cust_ids.shape
...
cust_ids.to_csv("indirim_hedef_mü?teri_ids.csv", index=False)
